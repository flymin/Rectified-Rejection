...
def attack_pgd(
        model, X, y, epsilon, alpha, attack_iters, restarts, norm,
        adaptive_evidence=False, adaptive_lambda=1., uniform_lambda=False,
        BNeval=False, twobranch=False, twosign=False):
    if BNeval:
        model.eval()
    ...
    for _ in range(restarts):
        ...
        for _ in range(attack_iters):
            ...
            if norm == "l_inf":
                ...
                d = (delta + scaled_g * alpha).view(delta.size(0), -1).renorm(p=2,
                                                                              dim=0, maxnorm=epsilon).view_as(delta)
...
def attack_trades(
        model, X, y, epsilon, alpha, attack_iters, restarts, norm, BNeval=True,
        twobranch=False):
    model.eval()
    ...
    for _ in range(attack_iters):
        output = model(normalize(X + delta)
                       )[0] if twobranch else model(normalize(X + delta))
...
def attack_ATRO(
        MHRLoss, num_cla, model, X, y, epsilon, alpha, attack_iters, restarts,
        norm, BNeval=True, twobranch=True):
    model.eval()
    delta = torch.zeros_like(X).uniform_(-epsilon, epsilon).cuda()
    delta = clamp(delta, lower_limit - X, upper_limit - X)
    delta.requires_grad = True
...
def attack_CARL(
        model, X, y, epsilon, alpha, attack_iters, restarts, norm, BNeval=False,
        twobranch=True):
    if BNeval:
        model.eval()
    delta = torch.zeros_like(X).uniform_(-epsilon, epsilon).cuda()
...
def attack_ccat(
        model, X, y, epsilon, alpha, attack_iters, restarts, norm, BNeval=False,
        twobranch=False, beta=0.9, lr_decay=1.5):
    if BNeval:
        model.eval()
    max_loss = torch.zeros(y.shape[0]).cuda()
    max_delta = torch.zeros_like(X).cuda()
    ...
    for _ in range(restarts):
        ...
        for ai in range(attack_iters):
            ...
            if norm == "l_inf":
                ...
                momentum_grad = beta * momentum_grad + \
                    (1 - beta) * torch.sign(grad)
                d = torch.clamp(
                    delta + alpha_batch * momentum_grad, min=-epsilon,
                    max=epsilon)
            elif norm == "l_2":
                g_norm = torch.norm(
                    grad.view(grad.shape[0], -1), dim=1).view(-1, 1, 1, 1)
                scaled_g = grad / (g_norm + 1e-10)
                d = (delta + scaled_g * alpha_batch).view(delta.size(0), -1).renorm(p=2,
                                                                                    dim=0, maxnorm=epsilon).view_as(delta)

            # backtrack
            d = clamp(d, lower_limit - X, upper_limit - X)
            output = model(normalize(X + d)
                           )[0] if twobranch else model(normalize(X + d))
            loss_d = F.cross_entropy(output.detach(), y, reduction='none')
            alpha_batch[loss_d <= best_loss] = alpha_batch[loss_d <=
                                                           best_loss] / lr_decay
...
def main():
    ...
    if args.model_name == 'PreActResNet18':
    ...
    elif args.model_name == 'PreActResNet18_twobranch_DenseV1': # use this
        model = PreActResNet18_twobranch_DenseV1(
            num_classes=num_cla, out_dim=args.out_dim, use_BN=args.useBN, along=along)
    elif args.model_name == 'PreActResNet18_twobranch_DenseV1Multi':
        model = PreActResNet18_twobranch_DenseV1Multi(
            num_classes=num_cla, out_dim=args.out_dim, use_BN=args.useBN, along=along)
    elif args.model_name == 'PreActResNet18_twobranch_DenseV2':
        model = PreActResNet18_twobranch_DenseV2(
            num_classes=num_cla, out_dim=args.out_dim, use_BN=args.useBN, along=along)
        ...
        model = WideResNet_twobranch_DenseV1(
            34, num_cla, widen_factor=10, dropRate=0.0, use_BN=args.useBN,
            along=along, out_dim=args.out_dim)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (data, target) in enumerate(train_batches):
            ...
            if args.selfreweightATRO:
                delta = attack_ATRO(MHRLoss, num_cla, model, X, y, epsilon,
                                    pgd_alpha, args.attack_iters, args.restarts,
                                    args.norm, BNeval=True, twobranch=True)
            elif args.selfreweightCARL:
                delta = attack_CARL(model, X, y, epsilon, pgd_alpha, args.attack_iters,
                                    args.restarts, args.norm, BNeval=args.BNeval, twobranch=True)
            elif args.ATframework == 'TRADES':
                delta = attack_trades(
                    model, X, y, epsilon, pgd_alpha, args.attack_iters, args.
                    restarts, args.norm, BNeval=True, twobranch=args.twobranch)
                delta = delta.detach()
            elif args.ATframework == 'PGDAT':  # use pgd attack
                delta, adaptive_l = attack_pgd(
                    model, X, y, epsilon, pgd_alpha, args.attack_iters, args.
                ...
                output = model(
                    normalize(X))[0] if args.twobranch else model(
                    normalize(X))
                delta = attack_ccat(
                    model, X, y, epsilon, args.CCATstep / 255., args.CCATiter,
                    args.restarts, args.norm, BNeval=args.BNeval,
                    twobranch=args.twobranch)
                delta = delta.detach()
            # Standard training
            ...
            if args.ATframework == 'PGDAT':  # calculate loss
                ...
                output = model(
                    normalize(X))[0] if args.twobranch else model(
                    normalize(X))
                KL_term = criterion_kl(
                    F.log_softmax(robust_output, dim=1),
                    F.softmax(output, dim=1))
                robust_loss = criterion(output, y) + args.TRADESlambda * KL_term
            elif args.ATframework == 'CCAT':
                sl = 1 - torch.max(delta.detach().view(y.size(0), -1).abs(),
                                   dim=1, keepdim=True)[0] / (args.CCATscale * epsilon)
                sl = torch.pow(sl, args.CCATrho)  # bs x num_cla
                # print(sl)
                smoothed_label = sl * F.one_hot(y,
                                                num_classes=num_cla) + (1 - sl) / num_cla
                robust_loss = criterion(
                    output, y) + args.CCATratio * criterion_kl(
                    F.log_softmax(robust_output, dim=1),
                    smoothed_label.float())

            if args.adaptivetrain:
                if args.selfreweightCalibrate:  # next here
                    robust_output_s = torch.softmax(
                        robust_output * args.tempC, dim=1)
                    robust_con_pre, robust_con_label = robust_output_s.max(
                        1)  # predicted label and confidence
                    ...
                    robust_con_y = robust_output_s_[
                        ...
                        y].detach()  # predicted prob on the true label y
                    ...
                    robust_loss += args.adaptivetrainlambda * aux_loss.mean(
                        dim=0)
                    ...
                    if wrong_index.size(0) > 0 and correct_index.size(0) > 0:
                        ...
                        L_en = torch.pow(
                            F.relu(logp_robust_wrong - args.m_out),
                            2).mean() + torch.pow(
                            F.relu(args.m_in - logp_robust_correct),
                            2).mean()
                        robust_loss += args.adaptivetrainlambda * L_en

                elif args.selfreweightATRO:
                    robust_output_aux = robust_output_aux.tanh()  # -1 to 1
                    robust_loss += args.ATRO_coefficient * MHRLoss(
                        F.softmax(robust_output, dim=1),
                        robust_output_aux, y, num_cla)[0]
                    #robust_loss += args.ATRO_coefficient * MHRLoss(robust_output, robust_output_aux, y, num_cla)[0]
                    #robust_loss += 0.5 * WeightPenalty()(model)

                elif args.selfreweightCARL:
                    ro_output_all = torch.cat(
                        (robust_output, robust_output_aux),
                        dim=1)  # bs x 11 or bs x 101
                    ro_softmax_output = F.softmax(ro_output_all, dim=1)
                    ro_so_y = ro_softmax_output[torch.tensor(
                        range(X.size(0))), y]
                    ro_so_a = ro_softmax_output[torch.tensor(
                        range(X.size(0))), -1]
                    ...
                    grad_CE = torch.autograd.grad(
                        CW_loss(output, y, SUM=True),
                        X, create_graph=True)[0]
                    grad_norm = torch.norm(
                        grad_CE.view(X.size(0), -1), p=1, dim=1)
        ...
        for i, (data, target) in enumerate(test_batches):
            ...
            delta, _ = attack_pgd(model, X, y, epsilon, pgd_alpha, args.attack_iters,
            ...
            if args.twobranch:
                ...
                if args.selfreweightCalibrate:
                    ...
                    # bs x 1, Calibration function A \in [0,1]
                    test_robust_evi_all = robust_output_aux.tanh().squeeze()
                    ...
                    softmax_output = F.softmax(output_all, dim=1)
                    ro_softmax_output = F.softmax(ro_output_all, dim=1)
                    test_evi_all = softmax_output[torch.tensor(
                        range(X.size(0))), -1]
                    test_robust_evi_all = ro_softmax_output[torch.tensor(
                        range(X.size(0))), -1]
    ...
    if True:
        ...
        for i, (data, target) in enumerate(test_batches):
            ...
            if args.target:
                ...
                delta, _ = attack_pgd(
                    model, X, y_target, epsilon, pgd_alpha, args.attack_iters,
                    args.restarts, args.norm, target=True,
                    twobranch=args.twobranch)
            else:
                delta, _ = attack_pgd(
                    model, X, y, epsilon, pgd_alpha, args.attack_iters, args.
